...
/**
 * Notification Component - alert messages
 *
 * Vai trò: UI thuần, hiển thị success/error/warning messages
...
export default function Notification({ type = "info", message, onClose }) {
  ...
  return (
    <div
      className={`relative px-6 py-4 border rounded-lg ${styles[type] || styles.info} flex items-start gap-3`}
      role="alert"
    >
